...
loadstring(game:HttpGet("https://raw.githubusercontent.com/gotham10/versions/main/v4.lua"))()
-- ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
local function runAutoSellLimited()
    ...
    local thread = coroutine.create(function()
        while _G.Config.AutoSellLimited and task.wait(3) do
            pcall(updateAutoSell)
        end
    end)
    _G.ScriptManager.Threads["AutoSellLimited"] = thread
