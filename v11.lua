...
local function runAutoBuyHalloweenCrate()
	...
	if not _G.Config.AutoBuyHalloweenCrate or not _G.Config.HalloweenItemRegistry then return end

	local connections = {}
	_G.ScriptManager.Connections["AutoBuyHalloweenCrate"] = connections

	local buyCrateRemote = remotes:WaitForChild("BuyCrate", 5)
	local candyLabel = playerGui.Main:FindFirstChild("Event_Candy") and playerGui.Main.Event_Candy:FindFirstChild("CurrentFrame") and playerGui.Main.Event_Candy.CurrentFrame:FindFirstChild("Amount")

	if not buyCrateRemote or not candyLabel then return end

	local sortedRegistry = {}
	for _, item in pairs(_G.Config.HalloweenItemRegistry) do
		table.insert(sortedRegistry, item)
	end
	table.sort(sortedRegistry, function(a, b) return a.Cost > b.Cost end)

	local function parseAmount(text)
		return tonumber(text:gsub(",", "")) or 0
	end

	local function onCandyChanged()
		if not _G.Config.AutoBuyHalloweenCrate then return end
		
		local amount = parseAmount(candyLabel.Text)
		
		for _, item in ipairs(sortedRegistry) do
			if item.Enabled and amount >= item.Cost then
				pcall(buyCrateRemote.FireServer, buyCrateRemote, item.Name)
				break
			end
		end
	end

	table.insert(connections, candyLabel:GetPropertyChangedSignal("Text"):Connect(onCandyChanged))
	pcall(onCandyChanged)
end
