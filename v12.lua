...
local function runAutoAcceptGifts()
    ...
    local conn = giftItemRemote.OnClientEvent:Connect(function(giftPayload)
        if not _G.Config.AutoAcceptGGifts then return end
...
local function runHalloweenBuyer()
    cleanup("HalloweenBuyer")
    if not _G.Config.AutoHalloweenBuyer or not _G.Config.halloweenRegistryTable then return end

    local connections = {}
    _G.ScriptManager.Connections["HalloweenBuyer"] = connections

    local buyRemote = remotes:WaitForChild("BuyCrate", 5)
    local candyLabel = playerGui:FindFirstChild("Main") 
                    and playerGui.Main:FindFirstChild("Event_Candy") 
                    and playerGui.Main.Event_Candy:FindFirstChild("CurrentFrame") 
                    and playerGui.Main.Event_Candy.CurrentFrame:FindFirstChild("Amount")

    if not buyRemote or not candyLabel then return end

    local function getCandyAmount()
        if not candyLabel then return 0 end
        local text = candyLabel.Text or "0"
        local num = tonumber(text:gsub("[,%s]", ""))
        return num or 0
    end

    local function attemptBuys()
        if not _G.Config.AutoHalloweenBuyer then return end
        ...
        while keepTrying do
            ...
            for itemName, itemInfo in pairs(_G.Config.halloweenRegistryTable) do
                if itemInfo.Buy and currentCandy >= itemInfo.Price then
                    if itemInfo.Price > itemPrice then
                        itemToBuy = itemName
            ...
            if itemToBuy then
                pcall(buyRemote.FireServer, buyRemote, itemToBuy)
                currentCandy = currentCandy - itemPrice
                keepTrying = true
                task.wait()
            else
                if currentCandy > 0 then
                    local lowestPriceItem = nil
                    local lowestPrice = math.huge
                    for itemName, itemInfo in pairs(_G.Config.halloweenRegistryTable) do
                        if itemInfo.Buy and itemInfo.Price < lowestPrice and currentCandy >= itemInfo.Price then
                            lowestPriceItem = itemName
                            lowestPrice = itemInfo.Price
                        end
                    end
                    
                    if lowestPriceItem then
                        pcall(buyRemote.FireServer, buyRemote, lowestPriceItem)
                        currentCandy = currentCandy - lowestPrice
                        keepTrying = true
                        task.wait()
                    end
                end
    ...
    table.insert(connections, candyLabel:GetPropertyChangedSignal("Text"):Connect(attemptBuys))
    task.spawn(attemptBuys)
end
