...
local function runAutoSell()
    ...
    table.insert(connections, g.selected:GetPropertyChangedSignal("Enabled"):Connect(function()
        if not _G.Config.AutoSell then return end
        pcall(updateAutoSell)
    end))
    
    table.insert(connections, g.unselected:GetPropertyChangedSignal("Enabled"):Connect(function()
        if not _G.Config.AutoSell then return end
        pcall(updateAutoSell)
    end))
end
...
local function runAutoSecret()
    cleanup("AutoSecret")
    if not _G.Config.AutoSecret then return end

    local connections = {}
    _G.ScriptManager.Connections["AutoSecret"] = connections

    local autoSellRemote = remotes:WaitForChild("AutoSell")
    local brainrotsFolder = ws:WaitForChild("ScriptedMap"):WaitForChild("MissionBrainrots")
    local plotsFolder = ws:WaitForChild("Plots")
    
    local secretButton = mainGui:WaitForChild("AutoSell"):WaitForChild("Frame"):WaitForChild("Secret"):WaitForChild("TextButton")
    ...
    if not (autoSellRemote and brainrotsFolder and plotsFolder and selectedGradient and unselectedGradient) then
        return
    end
    ...
    table.insert(connections, brainrotsFolder.ChildAdded:Connect(checkBrainrots))
    table.insert(connections, brainrotsFolder.ChildRemoved:Connect(checkBrainrots))
    
    for _, model in ipairs(brainrotsFolder:GetChildren()) do
        local conn1 = model:GetAttributeChangedSignal("Brainrot"):Connect(checkBrainrots)
        local conn2 = model:GetAttributeChangedSignal("Plot"):Connect(checkBrainrots)
